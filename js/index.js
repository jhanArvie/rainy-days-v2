...
function updateCartAmount() {
  const cartAmountElement =
    document.querySelector(".cartAmount"); /* cart icon in nav bar */
...
function displayProducts(products, containerId) {
  const container =
    document.getElementById(
      containerId
    ); /*  productsContainerSale or productsContainerGender */
  container.innerHTML = "";
  ...
  products.forEach((product) => {
    ...
    const sizesOptions = product.sizes
      .map((size) => {
        const cartItemId = generateCartItemId(
          product.id,
          size
        ); /* Make unique IDs for sameproduct but diff sizes */
    ...
    productElement.innerHTML = `
            ...
            <a href="product/index.html?id=${product.id}"><h4>${
      product.title
    }</h4></a>
            ...
            <div class="product-amount" data-id="${product.id}">
                ...
                <div class="quantity">${
                  cart[generateCartItemId(product.id, product.sizes[0])]
                    ?.quantity || 0
                }</div>
    ...
    container.appendChild(
      productElement
    ); /* inserted the created child div to parent div */
  });
  ...
  container.querySelectorAll(".fa-plus").forEach((button) => {
    button.addEventListener("click", () => {
      const productId =
        button.parentElement.getAttribute(
          "data-id"
        ); /* return the ID of specific product */
      const selectedSize =
        button.parentElement.previousElementSibling.querySelector(
          ".select-size"
        ).value; /* return the selectedSize */
      const cartItemId = generateCartItemId(
        productId,
        selectedSize
      ); /* took the value of productID, selectedSize to generate it to local storage when clickd */
      ...
      cart[cartItemId] = cart[cartItemId] || {
        ...productsArray.find((p) => p.id === productId),
        quantity: 0,
        selectedSize: selectedSize,
      };
      cart[cartItemId]
        .quantity++; /* after adding the quantity KeyboardEvent,,, add 1 to value of it */
      ...
      button.previousElementSibling.textContent =
        cart[
          cartItemId
        ].quantity; /* targets the elementsibling before this element */
    });
  });
  ...
  container.querySelectorAll(".fa-minus").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      const selectedSize =
        button.parentElement.previousElementSibling.querySelector(
          ".select-size"
        ).value; /* took the value of productID, selectedSize to generate it to local storage when clickd */
      ...
      if (cart[cartItemId] && cart[cartItemId].quantity > 0) {
        ...
        button.nextElementSibling.textContent =
          cart[cartItemId]?.quantity ||
          0; /* targets the elementsibling after this element */
      }
    });
  });
}
...
function filterByGender(gender) {
  ...
  if (gender !== "All") {
    /* "Male or "Female"  is not equal to "All" then filter my products */
...
document.getElementById("allBtn").addEventListener("click", function () {
  filterByGender(
    "All"
  ); /* All products are displayed if gender condition isn't met */
...
document.getElementById("maleBtn").addEventListener("click", function () {
  filterByGender(
    "Male"
  ); /* gender parameter is met to display filtered products */
  setActiveButton(this); /* add active class to this allBtn id when clicked */
});
